...
import { BarChart3, TrendingUp, Award, AlertCircle, Users, BookOpen, Target, Activity } from 'lucide-react';
...
const ResultAnalytics = ({ data }) => {
    ...
    const stats = useMemo(() => {
        ...
        const studentsWithResults = data.filter(s => s.result && s.result.Status !== 'Drop');
        ...
        const gpaStats = Object.entries(gpaAnalysis).map(([sem, gpas]) => {
        ...
        data.forEach(student => {
            if (student.result && student.result['Failed Subs'] && student.result['Failed Subs'] !== '-') {
        ...
        const performanceDistribution = studentsWithResults.reduce((acc, student) => {
            if (!student.result) return acc;
        ...
        const studentFailures = data
            ...
            .map(s => {
                const failedCount = s.result['Failed Subs'] && s.result['Failed Subs'] !== '-' 
                    ? s.result['Failed Subs'].split(', ').length 
                    : 0;
                return { name: s.name, roll: s.roll, failedCount, subjects: s.result['Failed Subs'] };
            })
 ...
 const exportToPDF = async () => {
        ...
        try {
            ...
            pdf.save(`RPI-Result-Analysis-${new Date().toISOString().split('T')[0]}.pdf`);
        } catch (error) {
    ...
    //                             <h3 className="text-sm font-medium text-blue-300">Total Students</h3>
    //                         </div>
    ...
    //                             <h3 className="text-sm font-medium text-green-300">Passed</h3>
    //                         </div>
    ...
    //                             <h3 className="text-sm font-medium text-yellow-300">Dropped</h3>
    //                         </div>
    //                         <p className="text-2xl font-bold text-white">
    ...
    //                                     <span className="text-green-400">Excellent (≥3.5 GPA)</span>
    //                                     <span className="text-gray-300">{stats.performanceDistribution.excellent} students</span>
    //                                 </div>
    ...
    //                             <div>
    //                                 <div className="flex justify-between text-sm mb-1">
    //                                     <span className="text-blue-400">Good (3.0-3.49 GPA)</span>
    //                                     <span className="text-gray-300">{stats.performanceDistribution.good} students</span>
    //                                 </div>
    //                                 <div className="w-full bg-gray-700 rounded-full h-3">
    ...
    //                             <div>
    //                                 <div className="flex justify-between text-sm mb-1">
    //                                     <span className="text-yellow-400">Average (2.5-2.99 GPA)</span>
    //                                     <span className="text-gray-300">{stats.performanceDistribution.average} students</span>
    //                                 </div>
    //                                 <div className="w-full bg-gray-700 rounded-full h-3">
    ...
    //                                     <span className="text-gray-300">{stats.performanceDistribution.poor} students</span>
    //                                 </div>
    //                                 <div className="w-full bg-gray-700 rounded-full h-3">
    ...
    //                     <div className="bg-gray-800 p-5 rounded-lg border border-gray-700">
    //                         <h3 className="text-lg font-semibold mb-4 text-white">Result Overview</h3>
    ...
    //                                                     <circle cx="50" cy="50" r={radius} fill="none" stroke="#10b981" strokeWidth="15"
    //                                                         strokeDasharray={`${passedLength} ${circumference - passedLength}`}
    //                                                         strokeDashoffset={circumference / 4} transform="rotate(-90 50 50)" />
    //                                                 )}
    //                                                 {stats.failedPercentage > 0 && (
    //                                                     <circle cx="50" cy="50" r={radius} fill="none" stroke="#ef4444" strokeWidth="15"
    //                                                         strokeDasharray={`${failedLength} ${circumference - failedLength}`}
    //                                                         strokeDashoffset={circumference / 4 - passedLength} transform="rotate(-90 50 50)" />
    //                                                 )}
    //                                                 {stats.droppedPercentage > 0 && (
    //                                                     <circle cx="50" cy="50" r={radius} fill="none" stroke="#f59e0b" strokeWidth="15"
    //                                                         strokeDasharray={`${droppedLength} ${circumference - droppedLength}`}
    //                                                         strokeDashoffset={circumference / 4 - passedLength - failedLength} transform="rotate(-90 50 50)" />
    //                                                 )}
    //                                             </>
    //                                         );
    //                                     })()}
    //                                     <text x="50" y="50" textAnchor="middle" dominantBaseline="middle" className="text-xl font-bold" fill="#ffffff">
    //                                         {Math.round(stats.passedPercentage)}%
    //                                     </text>
    //                                 </svg>
    //                             </div>
    ...
    //                     <h3 className="text-lg font-semibold mb-4 text-white">Performance by Shift</h3>
    //                     <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
    //                         {Object.entries(stats.shiftPerformance).map(([shift, perf]) => {
    //                             const total = perf.passed + perf.failed + perf.dropped;
    //                             const passRate = total > 0 ? (perf.passed / total * 100).toFixed(1) : 0;
    //                             return (
    //                                 <div key={shift} className="bg-gray-700 p-4 rounded-lg">
    //                                     <h4 className="font-semibold text-white mb-3">Shift {shift}</h4>
    ...
    //                                                 <span className="text-blue-400">Pass Rate:</span>
    ...
    //                                     <span className="text-gray-400">{count} ({((count/stats.totalStudents)*100).toFixed(1)}%)</span>
    //                                 </div>
    ...
    //                     <div className="bg-gray-800 p-5 rounded-lg border border-gray-700">
    //                         <h3 className="text-lg font-semibold mb-4 text-white">Semester Distribution</h3>
    ...
    //                                     <th className="text-left py-3 px-2 text-gray-300">Semester</th>
    //                                     <th className="text-right py-3 px-2 text-gray-300">Students</th>
    //                                     <th className="text-right py-3 px-2 text-gray-300">Average</th>
    //                                     <th className="text-right py-3 px-2 text-gray-300">Highest</th>
    //                                     <th className="text-right py-3 px-2 text-gray-300">Lowest</th>
    //                                     <th className="text-right py-3 px-2 text-gray-300">Median</th>
    //                                 </tr>
    //                             </thead>
    //                             <tbody>
    //                                 {stats.gpaStats.map((stat) => (
    //                                     <tr key={stat.semester} className="border-b border-gray-700 hover:bg-gray-750">
    //                                         <td className="py-3 px-2 text-white font-medium">{stat.semester}</td>
    //                                         <td className="py-3 px-2 text-right text-gray-300">{stat.count}</td>
    //                                         <td className="py-3 px-2 text-right">
    //                                             <span className={`font-semibold ${
    //                                                 stat.average >= 3.5 ? 'text-green-400' :
    //                                                 stat.average >= 3.0 ? 'text-blue-400' :
    //                                                 stat.average >= 2.5 ? 'text-yellow-400' : 'text-red-400'
    //                                             }`}>
    //                                                 {stat.average.toFixed(2)}
    //                                             </span>
    //                                         </td>
    //                                         <td className="py-3 px-2 text-right text-green-400 font-semibold">{stat.highest.toFixed(2)}</td>
    //                                         <td className="py-3 px-2 text-right text-red-400 font-semibold">{stat.lowest.toFixed(2)}</td>
    //                                         <td className="py-3 px-2 text-right text-gray-300">{stat.median.toFixed(2)}</td>
    //                                     </tr>
    //                                 ))}
    //                             </tbody>
    //                         </table>
    ...
    //                 {/* GPA Visualization */}
    //                 <div className="bg-gray-800 p-5 rounded-lg border border-gray-700">
    //                     <h3 className="text-lg font-semibold mb-4 text-white">Average GPA by Semester</h3>
    //                     <div className="h-64 flex items-end justify-around gap-2">
    //                         {stats.gpaStats.map((stat) => {
    //                             const height = (stat.average / 4) * 100;
    //                             return (
    //                                 <div key={stat.semester} className="flex-1 flex flex-col items-center">
    //                                     <div className="text-xs text-gray-400 mb-1">{stat.average.toFixed(2)}</div>
    //                                     <div className="w-full bg-gray-700 rounded-t relative" style={{ height: `${height}%` }}>
    //                                         <div className={`w-full h-full rounded-t ${
    //                                             stat.average >= 3.5 ? 'bg-green-500' :
    //                                             stat.average >= 3.0 ? 'bg-blue-500' :
    //                                             stat.average >= 2.5 ? 'bg-yellow-500' : 'bg-red-500'
    //                                         }`}></div>
    //                                     </div>
    //                                     <div className="text-xs text-gray-300 mt-2">{stat.semester}</div>
    //                                 </div>
    //                             );
    //                         })}
    //                     </div>
    ...
    //                                     index === 0 ? 'bg-red-600' :
    //                                     index === 1 ? 'bg-red-500' :
    //                                     index === 2 ? 'bg-red-400' : 'bg-gray-700'
    //                                 }`}>
    //                                     {index + 1}
    //                                 </div>
    ...
    //                                         <span className="text-red-400 font-semibold">{count} failures</span>
    //                                     </div>
    //                                     <div className="w-full bg-gray-700 rounded-full h-2.5">
    //                                         <div className="bg-red-500 h-2.5 rounded-full transition-all" 
    //                                             style={{ width: `${(count / stats.resultStats.failed) * 100}%` }}>
    //                                         </div>
    //                                     </div>
    //                                 </div>
    //                             </div>
    ...
    //                 <div className="bg-gray-800 p-5 rounded-lg border border-gray-700">
    //                     <h3 className="text-lg font-semibold mb-4 text-white">Subject Failure Impact</h3>
    //                     <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-2">
    //                         {stats.topFailedSubjects.slice(0, 15).map(([subject, count]) => {
    //                             const intensity = Math.min((count / stats.resultStats.failed) * 100, 100);
    //                             return (
    //                                 <div key={subject} 
    ...
    //                                     }}>
    //                                     <div className="text-xs font-mono text-white mb-1">{subject}</div>
    //                                     <div className="text-lg font-bold text-white">{count}</div>
    //                                     <div className="text-xs text-gray-300">{((count/stats.resultStats.failed)*100).toFixed(1)}%</div>
    //                                 </div>
    //                             );
    //                         })}
    //                     </div>
    ...
    //                                     <th className="text-left py-3 px-2 text-gray-300">Roll</th>
    //                                     <th className="text-left py-3 px-2 text-gray-300">Name</th>
    //                                     <th className="text-right py-3 px-2 text-gray-300">Avg GPA</th>
    //                                     <th className="text-right py-3 px-2 text-gray-300">Semesters</th>
    //                                 </tr>
    //                             </thead>
    //                             <tbody>
    //                                 {stats.topPerformers.map((student, index) => (
    //                                     <tr key={student.roll} className="border-b border-gray-700 hover:bg-gray-750">
    ...
    //                                             </div>
    //                                         </td>
    //                                         <td className="py-3 px-2 text-gray-300 font-mono">{student.roll}</td>
    //                                         <td className="py-3 px-2 text-white">{student.name}</td>
    //                                         <td className="py-3 px-2 text-right">
    //                                             <span className="text-green-400 font-bold text-lg">{student.avgGPA}</span>
    //                                         </td>
    //                                         <td className="py-3 px-2 text-right text-gray-300">{student.count}</td>
    //                                     </tr>
    //                                 ))}
    //                             </tbody>
    //                         </table>
    ...
    //                             <div key={student.roll} className="bg-gray-750 p-4 rounded-lg border border-gray-700">
    ...
    //                                                 <div className="text-xs text-gray-400 font-mono">Roll: {student.roll}</div>
    //                                             </div>
    ...
    //                         <h4 className="text-sm font-medium text-purple-300 mb-2">Perfect Scores</h4>
    //                         <p className="text-3xl font-bold text-white">
    //                             {stats.topPerformers.filter(s => parseFloat(s.avgGPA) === 4.0).length}
    //                         </p>
    //                         <p className="text-xs text-purple-300 mt-1">Students with 4.0 GPA</p>
    //                     </div>
    ...
    //                         <p className="text-3xl font-bold text-white">
    //                             {stats.studentFailures.filter(s => s.failedCount >= 4).length}
    //                         </p>
    //                         <p className="text-xs text-orange-300 mt-1">Failed 4+ subjects</p>
    //                     </div>
    //                     <div className="bg-gradient-to-br from-teal-900 to-teal-800 p-5 rounded-lg border border-teal-700">
    //                         <h4 className="text-sm font-medium text-teal-300 mb-2">Strong Performers</h4>
    //                         <p className="text-3xl font-bold text-white">
    //                             {stats.topPerformers.filter(s => parseFloat(s.avgGPA) >= 3.5).length}
    //                         </p>
    ...
    return (
        <div className="bg-gray-900 text-gray-100 rounded-lg shadow-lg mb-6 border border-gray-800 p-6">
...
<div className="bg-gray-900 text-gray-100 p-6 rounded-lg shadow-lg mb-6 border border-gray-800">
            ...
            {selectedView === 'students' && (
                ...
                <div className="text-center text-xs text-gray-600 mt-12 pt-4 border-t border-gray-800 print-only">
                    <p>Generated by RPI Result Analyzer • {new Date().toLocaleDateString()}</p>
                    <p className="mt-1">© {new Date().getFullYear()} RPI Computer Center</p>
                </div>
            </div>
        </div>
    );
